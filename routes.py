...
def del_post_image(mapper, connection, target):
    if target.path:
        try:
            print(target.id)
...
def del_cert_image(mapper, connection, target):
    if target.path:
        try:
            print(target.id)
            cert_path = cert() if callable(cert) else cert
            os.remove(op.join(cert_path, target.path))
        except OSError:
            pass
